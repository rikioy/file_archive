...
func add(path, name, dateStr string) (err error) {
	...
	var realpath string
	if realpath, err = insert(path, dstPath, dbFile, name); err != nil {
		return
	}
	log.Infof("插入文件成功: %s/%s => %s", path, name, realpath)
	return
}
...
func deal(v fileinfo) bool {
	...
	filePath := v.Path + Sep + v.Info.Name()
	...
	*/
	...
	for _, v := range files {
		if deal(v) {
			...
			errCount++
			filecopy(v.Path+Sep+v.Info.Name(), FailPath+Sep+v.Info.Name())
		}
	}
